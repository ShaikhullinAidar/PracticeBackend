...
namespace Sockets
{
    ...
    public class AsynchronousSocketListener
    {
        ...
        private static byte[] ProcessRequest(Request request)
        {
            ...
            var path = request.RequestUri.Split("?")[0];
            var arg = String.Empty;
            if (request.RequestUri.Contains('?'))
            {
                arg = request.RequestUri.Split("?")[1];
            }
            switch (path)
            {
                case "/time.html":
                {
                    var t = Encoding.UTF8.GetBytes(Encoding.UTF8.GetString(File.ReadAllBytes("time.template.html")).Replace("{{ServerTime}}", DateTime.Now.ToString()));
                    body = Encoding.UTF8.GetBytes($"HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=utf-8\r\nContent-Length: &{t.Length}\r\n\r\n").Concat(t).ToArray();
                    break;
                }
                case "/groot.gif":
                {
                    var groot = File.ReadAllBytes("groot.gif");
                    body = Encoding.UTF8.GetBytes($"HTTP/1.1 200 OK\r\nContent-Type: image/gif; charset=utf-8\r\nContent-Length: &{groot.Length}\r\n\r\n").Concat(groot).ToArray();
                    break;
                }
                case "/":
                case "/hello.html":
                {
                    var a = HttpUtility.ParseQueryString(arg);
                    var hello = File.ReadAllBytes("hello.html");
                    var helloAsStr = Encoding.UTF8.GetString(hello);
                    var cookieName = string.Empty;
                    if (a["greeting"] != null)
                    {
                        helloAsStr = helloAsStr.Replace("{{Hello}}", HttpUtility.HtmlEncode(a["greeting"]));
                    }
                    if (a["name"] != null)
                    {
                        helloAsStr = helloAsStr.Replace("{{World}}", HttpUtility.HtmlEncode(a["name"]));
                        cookieName = HttpUtility.UrlEncode(a["name"]);
                    }
                    else if (request.Cookie.ContainsKey("name"))
                    {
                        helloAsStr = helloAsStr.Replace("{{World}}", HttpUtility.HtmlEncode(HttpUtility.UrlDecode(request.Cookie["name"])));
                        cookieName = request.Cookie["name"];
                    }
                    hello = Encoding.UTF8.GetBytes(helloAsStr);
