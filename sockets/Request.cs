...
namespace Sockets
{
    public class Request
    {
        ...
        public static Request StupidParse(byte[] requestBytes)
        {
            ...
            var cookie = ParseCookie(requestString);
        ...
        private static Dictionary<string, string> ParseCookie(string requestString)
        {
            ...
            foreach (var header in headers)
            {
                ...
                {
                    var namesCookies = header.Split(' ').Skip(1).ToList();
                    foreach (var nameCookie in namesCookies)
                    {
                        var nameAndCookie = nameCookie.Split('=');
                        cookies[nameAndCookie[0]] = nameAndCookie[1].Substring(0, nameAndCookie[1].Length - 1);
                    }
                }
            }
